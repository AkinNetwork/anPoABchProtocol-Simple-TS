// src/demo.ts
import { AkinChain, makeTransaction } from "./core/chain";
import { DemoSessionApp } from "./apps/demoSession";

function demo() {
  const chainId = "AKIN:CHAIN:DEMO";
  const validators = ["AKIN:VALIDATOR:1"];
  const chain = new AkinChain(chainId, validators, [new DemoSessionApp()]);

  const req = ["AKIN:USER:1", "AKIN:ORG:AT", "AKIN:SYS:PAY"];

  // Create session
  const txCreate = makeTransaction(
    chainId,
    "demo_session",
    "AKIN:USER:1",
    {
  ...
  for (const signer of req.slice(0, 2)) {
    const tx = makeTransaction(
      ...
      "demo_session",
      signer,
      { action: "sign", serviceId: "SESSION-1" },
      signer
    );
  ...
  console.log(
    "state after block 1",
    JSON.stringify(chain.appStates["demo_session"], null, 2)
  );

  // Last signer
  const last = req[2];
  const txLast = makeTransaction(
    chainId,
    "demo_session",
    last,
    { action: "sign", serviceId: "SESSION-1" },
